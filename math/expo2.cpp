...
//a^b^c mod p =a^r mod p where r=b^c mod p-1
// we can wrire b^c =q(P-1) + r
//a^b^c =a^(q(p-1)+r) mod p and from fermat little theorem its a^r mod p 

void solve(){
    int a,b,c;
    cin>>a>>b>>c;
    int e = modpow(b,c,MOD-1);
    cout << modpow(a,e,MOD) << endl;
}
